    ...
    <div class="card ">
        <div class="card-header d-flex flex-row align-content-center">
            ...
            <div class="card-tools d-flex flex-row">
                ...
                @can('category-create')
                    <a href="{{route('categories.create')}}" class="btn btn-success btn-sm m-1">
                        Add
                    </a>
                @endcan
                        ...
                        @foreach($latestCategories as $category)
                            <tr>
                                ...
                                <td class="project-actions text-center">
                                    @can('category-update')
                                        <a class="btn btn-info btn-sm m-1" href="{{route('categories.edit', ['id' => $category->id])}}">
                                    ...
                                    @endcan
                                    @can('category-delete')
                                        <a class="btn btn-danger btn-sm m-1" data-action="btnDelete" data-name="{{$category->name}}" data-url="{{route('categories.destroy', ['id'=> $category->id])}}" >
        ...
        <div class="col-md-5 mt-3">
            {{$latestCategories->appends(request()->query())->links()}}
        </div>
        <!-- /.card-body -->
    </div>
    <!-- /.card -->
