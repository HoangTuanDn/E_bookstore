...
@section('content')
    ...
    @include('partials.breadcrumb',['module' => 'roles', 'action' => 'edit'])
                                        ...
                                        <div class="card bg-light mb-3" style="max-width: 100%;">
                                            <div class="card-header">
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input check-warapper"
                                                           name="permission_group" value="{{$permission->id}}"
                                                           type="checkbox" id="inline{{$permission->key_code}}"
                                                           value="option1">
                                                ...
                                                @foreach($permission->permissions as $ofPermission)
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input check-children"
                                                               {{$role->permissions->contains('id', $ofPermission->id) ? 'checked' : ''}}
                                                               value="{{$ofPermission->id}}"
                                                               name="permission[]" type="checkbox"
                                                               id="inlineCheckbox{{$ofPermission->key_code}}"
                                                               value="option1"
                                                        >
                                                        <label class="form-check-label"
                                                               for="inlineCheckbox1">{{$ofPermission->name}}</label>
                                                    </div>
                                                @endforeach
                                            </div>
